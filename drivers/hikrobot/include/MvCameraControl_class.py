    ...
    if platform.system() == "Windows":
        if explicit_library:
            if "winmode" in ctypes.WinDLL.__init__.__code__.co_varnames:
                return WinDLL(explicit_library, winmode=0)
            return WinDLL(explicit_library)
        dllname = "MvCameraControl.dll"
        if "winmode" in ctypes.WinDLL.__init__.__code__.co_varnames:
            return WinDLL(dllname, winmode=0)
        return WinDLL(dllname)

    if explicit_library:
        return CDLL(explicit_library)

    return CDLL("libMvCameraControl.so")


MvCamCtrldll = _load_mvs_library()
